...
export function supportsERC1155(token: Address): bool {
  ...
  let introspection_01ffc9a7 = supportsInterface(erc1155, '01ffc9a7'); // ERC165
  let introspection_d9b67a26 = supportsInterface(erc1155, 'd9b67a26'); // ERC1155
...
export function updateERC1155Balance(
  ...
  let tokenIdBalanceId = balanceId.concat('_').concat(tokenId.toString());
  // load tokenIdbalnce
  let erc1155TokenIdBalance = ERC1155TokenIdBalance.load(tokenIdBalanceId);
  let erc1155Created = false;
  ...
  if (!erc1155Balance) {
    ...
    erc1155Balance.balances = [erc1155TokenIdBalance.id];
  } else {
    // check if it was created and if so add it to balances
    if (erc1155Created) {
      erc1155Balance.balances = erc1155Balance.balances.concat([
