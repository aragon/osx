...
[NOTE]
====
Typically, the `ROOT_PERMISSION_ID` permission is granted only to the `DAO` contract itself. Contracts related to the Aragon infrastructure temporarily require it during the xref:framework/dao-creation.adoc[DAO creation] and xref:framework/plugin-setup-processor.adoc[plugin setup] processes. 
...
Exceptions are, again, the xref:framework/dao-creation.adoc[DAO creation] and xref:framework/plugin-setup-processor.adoc[plugin setup] processes.
