...
4. Sets the xref:core/permissions.adoc##permissions-native-to-the-dao-contract[native permissions] of the `DAO` and revokes its own ownership.
...
If the requested ENS `subdomain` name xref:framework/ens-registrar.adoc[is valid] and not taken, the `DAORegistry` registers the subdomain and adds the `DAO` contract address to the `DAORegistry`.
