...
contract GovernanceWrappedERC20Upgradeable is DaoAuthorizableUpgradeable, GovernanceWrappedERC20, UUPSUpgradeable {
    
    /// @notice The ID of the permission required to call the `_authorizeUpgrade` function.
    bytes32 public constant UPGRADE_GOVERNANCE_ERC20_PERMISSION_ID = keccak256("UPGRADE_GOVERNANCE_ERC20_PERMISSION");
    ...
    constructor(IERC20Upgradeable _token, string memory _name, string memory _symbol) GovernanceWrappedERC20(_token, _name, _symbol) {}

    
    /// @notice Internal method authorizing the upgrade of the contract via the [upgradeability mechanism for UUPS proxies](https://docs.openzeppelin.com/contracts/4.x/api/proxy#UUPSUpgradeable) (see [ERC-1822](https://eips.ethereum.org/EIPS/eip-1822)).
    /// @dev The caller must have the `UPGRADE_DAO_PERMISSION_ID` permission.
    function _authorizeUpgrade(address) internal virtual override auth(UPGRADE_GOVERNANCE_ERC20_PERMISSION_ID) {}
}