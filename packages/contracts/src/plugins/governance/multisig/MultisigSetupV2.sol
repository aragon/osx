...
contract MultisigSetupV2 is PluginSetup {
    ...
    function prepareInstallation(
        ...
        (address[] memory members, MultisigV2.MultisigSettings memory multisigSettings) = abi.decode(
            _data,
            (address[], MultisigV2.MultisigSettings)
        );

        // Prepare and Deploy the plugin proxy.
        plugin = createERC1967Proxy(
            address(multisigBase),
            abi.encodeWithSelector(MultisigV2.initialize.selector, _dao, members, multisigSettings)
        );
        ...
        returns (bytes memory initData, PreparedSetupData memory preparedSetupData)
    {
        ...
        permissions[0] = PermissionLib.MultiTargetPermission(
            ...
            address(new MultisigCondition(_payload.plugin)),
            multisigBase.CREATE_PROPOSAL_PERMISSION_ID()
        );
