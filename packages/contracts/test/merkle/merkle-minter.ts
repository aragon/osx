...
import { MerkleMinter, MerkleDistributor, DAO, GovernanceERC20 } from '../../typechain';
...
describe('MerkleDistributor', function () {
  ...
  beforeEach(async () => {
    ...
    await minter.initialize(dao.address, ethers.constants.AddressZero, token.address, distributor.address);
  ...
  describe('merkleMint:', () => {
    ...
    it('mints tokens using the merkle distributor', async () => {
      ...
      let clonedAddress = await minter.callStatic.merkleMint(merkleRoot, totalAmount, dummyMerkleTreeStorageLink, dummyMintingContext);

      // execute merkleMint and listen for the event containing the cloned address
      expect(
        await minter.merkleMint(merkleRoot, totalAmount, dummyMerkleTreeStorageLink, dummyMintingContext)
      )
        .to.emit(minter, 'MintedMerkle')
        .withArgs(clonedAddress, merkleRoot, totalAmount, dummyMerkleTreeStorageLink, dummyMintingContext);
    });
