...
skipTestSuiteIfNetworkIsZkSync('TokenVotingSetup', async () => {
  describe('TokenVotingSetup', function () {
    ...
    before(async () => {
      ...
      governanceERC20Base = await hre.wrapper.deploy('GovernanceERC20', {
      ...
      tokenVotingSetup = await hre.wrapper.deploy('TokenVotingSetup', {
    ...
    it('does not support the empty interface', async () => {
      expect(await tokenVotingSetup.supportsInterface('0xffffffff')).to.be
        .false;
    });
    ...
    it('creates token voting base with the correct interface', async () => {
      ...
      expect(
        await tokenVoting.supportsInterface(
          getInterfaceID(tokenVotingInterface)
        )
      ).to.be.eq(true);
    });
    ...
    describe('prepareInstallation', async () => {
      ...
      it('correctly sets up `GovernanceWrappedERC20` helper, when an ERC20 token address is supplied', async () => {
        ...
        const GovernanceWrappedERC20Factory =
          new GovernanceWrappedERC20__factory(signers[0]);
