...
describe('CounterPluginSetup(Example)', function () {
  ...
  let signers: any;
  let counterV1Manager: CounterV1PluginSetup;
  let counterV2Manager: CounterV2PluginSetup;
  ...
  before(async () => {
    ...
    const CounterV1Manager = await ethers.getContractFactory(
      'CounterV1PluginSetup'
    );
    ...
    const counterV1 = COUNTER_V1.attach(
      await counterV1Manager.multiplyHelperBase()
    );
  ...
  describe('getInstallInstruction', async () => {
    it('correctly returns permissions/helpers/plugin when helper is passed', async () => {
      ...
      const data = await counterV1Manager.getInstallInstruction(
        daoMock.address,
        salt,
        ownerAddress,
        params
