  ...
  describe(artifact, function () {
    ...
    describe('supportsInterface:', async () => {
      it('does not support the empty interface', async () => {
        expect(await governanceToken.supportsInterface('0xffffffff')).to.be
          .false;
      });
      ...
      it('it supports all inherited interfaces', async () => {
        await Promise.all(
          [
            ...
            expect(
              await governanceToken.supportsInterface(getInterfaceID(iface))
            ).to.be.true;
          })
        );
    ...
    describe('delegate', async () => {
      ...
      it('is checkpointed', async () => {
        ...
        expect(
          await governanceToken.getPastVotes(
            signers[0].address,
            tx1.blockNumber!
          )
        ).to.eq(0);
