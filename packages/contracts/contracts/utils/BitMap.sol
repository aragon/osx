...
function hasBit(uint256 allowFailureMap, uint8 index) pure returns (bool) {
    uint256 bitValue = allowFailureMap & (1 << (index & 0xff));
    return bitValue > 0;
}
...
function setBit(uint256 allowFailureMap, uint8 index) pure returns (uint256) {
