...
interface IPluginRepo {
    /// @notice Create new version with contract `_pluginSetupAddress` and content `@fromHex(_contentURI)`
    /// @param _newSemanticVersion Semantic version for new pluginRepo version
    /// @param _pluginSetupAddress Address for smart contract logic for version (if set to 0, it uses last versions' pluginSetupAddress)
