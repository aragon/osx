...
contract PluginSetupProcessor is DaoAuthorizable {
    ...
    event InstallationProcessed(address indexed dao, address indexed plugin, address[] helpers);
    ...
    function processInstallation(address _dao, PluginInstallParams calldata _params)
        external
        canProcessSetup(_dao, PROCESS_INSTALL_PERMISSION_ID)
    {
        bytes32 installedId = keccak256(abi.encode(_dao, _params.plugin));
        ...
        // important safety measure to include dao + plugin manager in the encoding.
        bytes32 installationId = keccak256(abi.encode(_dao, _params.pluginSetup, _params.plugin));

        helpersHashes[installationId] = keccak256(abi.encode(_params.helpers));
        ...
        emit InstallationProcessed(_dao, _params.plugin, _params.helpers);
    }
    ...
    function processUpdate(address _dao, PluginUpdateParams calldata _params)
        external
        canProcessSetup(_dao, PROCESS_UPDATE_PERMISSION_ID)
    {
        bytes32 updateId = keccak256(abi.encode(_dao, _params.plugin));

        if (isInstallationProcessed[updateId]) {
            revert PluginWithTheSameAddressExists(); // TODO: rename to Plugin not installed
        }

        // delete old helpers hashes
        bytes32 oldSetupId = keccak256(abi.encode(_dao, _params.oldPluginSetup, _params.plugin));

        if (helpersHashes[oldSetupId] != keccak256(abi.encode(_params.oldHelpers))) {
            revert HelpersMismatch();
        }
        ...
        // store new helpers hashes
        bytes32 newSetupId = keccak256(abi.encode(_dao, _params.newPluginSetup, _params.plugin));

        helpersHashes[newSetupId] = keccak256(abi.encode(_params.newHelpers));

        DAO(payable(_dao)).bulkOnMultiTarget(_params.permissions);

        emit UpdateProcessed(_dao, _params.plugin); // TODO: some other parts might be needed..
    }

    function processUninstallation(address _dao, PluginUninstallParams calldata _params)
        external
        canProcessSetup(_dao, PROCESS_UNINSTALL_PERMISSION_ID)
    {
        ...
        // Reverts if pluginSetup doesn't exist on the repo...
        _params.pluginSetupRepo.getVersionByPluginSetup(_params.pluginSetup);

        bytes32 setupId = keccak256(abi.encode(_dao, _params.pluginSetup, _params.plugin));

        if (helpersHashes[setupId] != keccak256(abi.encode(_params.activeHelpers))) {
            revert HelpersMismatch();
        }
        ...
        emit PluginUninstalled(_dao, _params.plugin);
    }
