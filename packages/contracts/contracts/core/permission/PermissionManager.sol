...
contract PermissionManager is Initializable {
    ...
    function applySimplePermissionsBulk(address _where, PermissionLib.SimplePermission[] calldata items)
        external
        auth(_where, ROOT_PERMISSION_ID)
    {
        for (uint256 i; i < items.length; ) {
            PermissionLib.SimplePermission memory item = items[i];
    ...
    /// @param items The array of bulk items to process.
    function applyAdvancedPermissionsBulk(PermissionLib.AdvancedPermission[] calldata items) external {
        for (uint256 i; i < items.length; ) {
            PermissionLib.AdvancedPermission memory item = items[i];
