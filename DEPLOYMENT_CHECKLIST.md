...
- [ ] Check if there is an official ENS deployment for the choosen chain and if yes:
...
To deploy run `yarn deploy --network NETWORK` in `packages/contracts` and replace `NETWORK` with the correct network name (e.g. for mainnet it is `yarn deploy --network NETWORK`)
...
- [ ] Go to the blockchain explorer and verify that each address is verified
  ...
  - [ ] If it is a proxy try to activate the blockchain explorer' proxy feature
...
- [ ] Public a new version of `@aragon/core-contracts` (`./packages/contracts`) to NPM
- [ ] Public a new version of `@aragon/core-contracts-ethers` (`./packages/contracts`) to NPM
