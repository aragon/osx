...
///
/// Ports `osx-commons/contracts/test/utils/deployment/proxy-lib.ts` and adds
/// explicit `ProxyCreated` event emission, immutable `implementation()`
/// getter, init-revert propagation, and distinct addresses for consecutive
/// deploys.
contract ProxyLibTest is Test {
    ...
    address internal constant FAKE_DAO =
    ...
    function test_deployUUPSProxy_initializedWhenCalldataProvided() public {
        ...
        PluginUUPSUpgradeableMockBuild1 proxy = PluginUUPSUpgradeableMockBuild1(
            proxyAddr
        );
    ...
    function test_deployUUPSProxy_emitsProxyCreatedWithDeployedAddress()
        public
    {
        vm.recordLogs();
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (
                ...
                logs[i].emitter == address(uupsFactory)
            ) {
    ...
    function test_deployUUPSProxy_revertsIfInitCalldataReverts() public {
        ...
        bytes memory bogus = abi.encodeWithSelector(
            bytes4(keccak256("doesNotExist()"))
        );
        vm.expectRevert();
        uupsFactory.deployUUPSProxy(bogus);
    }

    function test_deployUUPSProxy_consecutiveDeploysProduceDistinctAddresses()
        public
    {
    ...
    function test_deployMinimalProxy_emitsProxyCreatedWithDeployedAddress()
        public
    {
        vm.recordLogs();
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (
                logs[i].topics[0] == expectedTopic &&
                logs[i].emitter == address(cloneableFactory)
            ) {
                address emittedProxy = abi.decode(logs[i].data, (address));
                assertEq(emittedProxy, proxy);
                found = true;
    ...
    function test_deployMinimalProxy_consecutiveDeploysProduceDistinctAddresses()
        public
    {
