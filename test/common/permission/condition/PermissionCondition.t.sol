...
} from "../../../mocks/commons/permission/condition/PermissionConditionUpgradeableMock.sol";
...
/// here). Closes the gaps from `TESTS.md` §9: negative `supportsInterface`
/// case, exact `protocolVersion()` return value, frozen `IPermissionCondition`
/// iface ID matches v1.0.0.
abstract contract PermissionConditionSharedTest is Test {
    ...
    function test_IPermissionCondition_hasSameInterfaceIdAsV1_0_0() public pure {
        assertEq(type(IPermissionCondition).interfaceId, IPERMISSION_CONDITION_V1_0_0_INTERFACE_ID);
    }
    ...
    function test_supportsInterface_IProtocolVersion() public view {
        assertTrue(conditionMock.supportsInterface(type(IProtocolVersion).interfaceId));
    }
...
contract PermissionConditionUpgradeableTest is PermissionConditionSharedTest {
    function _deployConditionMock() internal override returns (IPermissionConditionMock) {
        return IPermissionConditionMock(address(new PermissionConditionUpgradeableMock()));
    }
}
