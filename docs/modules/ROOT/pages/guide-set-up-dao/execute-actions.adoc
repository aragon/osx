...
Executing actions on behalf of the DAO is done through the `execute` function from the `DAO.sol` contract. This function allows us to link:https://github.com/aragon/osx/blob/develop/src/core/dao/DAO.sol[pass an array of actions] to be executed by the DAO contract itself.
...
To grant the `EXECUTE_PERMISSION` to an address, you'll want to call on the `PermissionManager` 's link:https://github.com/aragon/osx/blob/develop/src/core/permission/PermissionManager.sol#L105[grant function] and pass it 4 parameters:
