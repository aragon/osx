...
In this Quickstart guide, we will use the Aragon Foundry template to set up a plugin.

== Setting up your environment

We recommend using our link:https://github.com/aragon/osx-plugin-template-foundry[Foundry template] to get started. If you don't have
it installed, you can do so by running:

```bash
git clone github.com/aragon/osx-plugin-template-foundry
```

Once you have cloned the repository the first step is to add a `.env` file with your `ALCHEMY_API_KEY`, 
there is a link:https://github.com/aragon/osx-plugin-template-foundry/blob/main/.env.example[.env.example] file you can use as a template.
...
## One or multiple hex encoded private keys separated by commas `,` replacing the default accounts.
PRIVATE_KEY="0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80" # Default Anvil account 0 private key. DON'T USE FOR DEPLOYMENTS

## Alchemy RPC endpoint credentials
ALCHEMY_API_KEY="zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz"
```

Follow the template's `README.md` to install dependencies and build the project.

Once set up, you should have two files called `MyPlugin.sol` and `MyPluginSetup.sol` inside
...
For more information on how to use the template, you can check the link:https://github.com/aragon/osx-plugin-template-foundry/blob/main/README.md[README] and 
the link:https://github.com/aragon/osx-plugin-template-foundry/blob/main/USAGE_GUIDE.md[USAGE GUIDE].
